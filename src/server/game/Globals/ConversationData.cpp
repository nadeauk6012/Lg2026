...
void ConversationDataStoreMgr::LoadConversations()
{
    ...
    do
    {
        ...
        if (lastCreature != lastEntryCreature.end())
        {
            if (data.mapid == lastCreature->second->mapid)
            {
                ...
                if (distsq1 < 0.5f)
                {
                    ...
                    TC_LOG_ERROR(LOG_FILTER_SQL, "ConversationDataStoreMgr::LoadConversations() >> Table `conversation` have clone npc %u witch stay too close (dist: %f). original npc guid %lu. npc with guid %lu will be deleted.", entry, distsq1, lastCreature->second->guid, guid);
                    continue;
                }
            }
            else
        ...
        {
            TC_LOG_ERROR(LOG_FILTER_SQL, "LoadConversations >> Table `conversation` have conversation (GUID: " UI64FMTD ") that have wrong spawn mask " UI64FMTD " including not supported difficulty modes for map (Id: %u) spawnMasks[data.mapid]: %lu.", guid, data.spawnMask, data.mapid, spawnMasks[data.mapid]);
