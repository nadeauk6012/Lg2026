...
void EventObjectDataStoreMgr::LoadEventObjects()
{
    ...
    do
    {
        ...
        if (lastObject != lastEntry.end())
        {
            if (data.mapid == lastObject->second->mapid)
            {
                ...
                if (distsq1 < 0.5f)
                {
                    ...
                    TC_LOG_ERROR(LOG_FILTER_SQL, "LoadEventObjects >> Table `eventobject` have clone npc %u witch stay too close (dist: %f). original npc guid %lu. npc with guid %lu will be deleted.", entry, distsq1, lastObject->second->guid, guid);
                    continue;
                }
            }
            else
        ...
        {
            TC_LOG_ERROR(LOG_FILTER_SQL, "LoadEventObjects >> Table `eventobject` have eventobject (GUID: " UI64FMTD ") that have wrong spawn mask %lu including not supported difficulty modes for map (Id: %u) spawnMasks[data.mapid]: %u.", guid, data.spawnMask, data.mapid, spawnMasks[data.mapid]);
