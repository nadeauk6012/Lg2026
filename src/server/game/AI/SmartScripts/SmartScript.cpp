...
void SmartScript::ProcessAction(SmartScriptHolder& e, Unit* unit, uint32 var0, uint32 var1, bool bvar, const SpellInfo* spell, GameObject* gob)
{
    ...
    switch (e.GetActionType())
    {
        case SMART_ACTION_TALK:
        {
            ...
            TC_LOG_DEBUG(LOG_FILTER_DATABASE_AI, "SmartScript::ProcessAction: SMART_ACTION_TALK: talker: %s (GuidLow: %u), textGuid: %lu",
        ...
        case SMART_ACTION_SET_INST_DATA:
        {
            ...
            if (!instance)
            {
                TC_LOG_ERROR(LOG_FILTER_SQL, "SmartScript: Event %u attempt to set instance data without instance script. EntryOrGuid %ld", e.GetEventType(), e.entryOrGuid);
                break;
            }
        ...
        case SMART_ACTION_CALL_TIMED_ACTIONLIST:
        {
            ...
            {
                TC_LOG_ERROR(LOG_FILTER_SQL, "SmartScript: Entry %ld SourceType %u Event %u Action %u is using TARGET_NONE(0) for Script9 target. Please correct target_type in database.", e.entryOrGuid, e.GetScriptType(), e.GetEventType(), e.GetActionType());
                break;
            }
        ...
        case SMART_ACTION_SUMMON_CONVERSATION:
        {
            ...
            TC_LOG_DEBUG(LOG_FILTER_DATABASE_AI, "SmartScript::ProcessAction:: SMART_ACTION_SUMMON_CONVERSATION: Conversation %d", e.action.conversation.id);
        ...
        case SMART_ACTION_SUMMON_ARIATRIGGER:
        {
            ...
            TC_LOG_DEBUG(LOG_FILTER_DATABASE_AI, "SmartScript::ProcessAction:: SMART_ACTION_SUMMON_ARIATRIGGER: AreaTrigger %d", e.action.areatrigger.id);
        ...
        default:
            TC_LOG_ERROR(LOG_FILTER_SQL, "SmartScript::ProcessAction: Entry %ld SourceType %u, Event %u, Unhandled Action type %u", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType());
            break;
    }
    ...
    if (e.link && e.link != e.event_id)
    {
        ...
        if (linked.GetActionType() && linked.GetEventType() == SMART_EVENT_LINK)
            ...
            TC_LOG_DEBUG(LOG_FILTER_SQL, "SmartScript::ProcessAction: Entry %ld SourceType %u, Event %u, Link Event %u not found or invalid, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.link);
    }
}
...
void SmartScript::InstallTemplate(SmartScriptHolder const& e)
{
    ...
    if (mTemplate)
    {
        TC_LOG_ERROR(LOG_FILTER_SQL, "SmartScript::InstallTemplate: Entry %ld SourceType %u AI Template can not be set more then once, skipped.", e.entryOrGuid, e.GetScriptType());
        return;
    }
