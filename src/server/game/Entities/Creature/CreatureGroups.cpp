...
void FormationMgr::AddCreatureToGroup(ObjectGuid::LowType const& groupId, Creature* member)
{
    ...
    {
        TC_LOG_DEBUG(LOG_FILTER_UNITS, "Group found: %lu, inserting creature GUID: %u, Group InstanceID %u", groupId, member->GetGUID().GetGUIDLow(), member->GetInstanceId());
    ...
    else
    {
        ...
        TC_LOG_DEBUG(LOG_FILTER_UNITS, "Group not found: %lu. Creating new group.", groupId);
...
{
    TC_LOG_DEBUG(LOG_FILTER_UNITS, "Deleting member pointer to GUID: %lu from group %lu", group->GetId(), member->GetDBTableGUIDLow());
...
void FormationMgr::LoadCreatureFormations()
{
    ...
    do
    {
        ...
        // check data correctness
        {
            ...
            {
                TC_LOG_ERROR(LOG_FILTER_SQL, "creature_formations table leader guid %lu incorrect (not exist)", group_member->leaderGUID);
                delete group_member;
                continue;
            }
            ...
            {
                TC_LOG_ERROR(LOG_FILTER_SQL, "creature_formations table member guid %lu incorrect (not exist)", memberGUID);
                delete group_member;
                continue;
            }
        }
...
{
    TC_LOG_DEBUG(LOG_FILTER_UNITS, "CreatureGroup::AddMember: Adding unit GetDBTableGUIDLow %lu GUID: %u", member->GetDBTableGUIDLow(), member->GetGUID().GetGUIDLow());
...
{
    TC_LOG_DEBUG(LOG_FILTER_UNITS, "CreatureGroup::RemoveMember: Removing unit GetDBTableGUIDLow %lu GUID: %u entry %u", member->GetDBTableGUIDLow(), member->GetGUID().GetGUIDLow(), member->GetEntry());
