...
bool Corpse::Create(ObjectGuid::LowType guidlow, Player* owner)
{
    ...
    if (!IsPositionValid())
    {
        TC_LOG_ERROR(LOG_FILTER_PLAYER, "Corpse (guidlow %lu, owner %s) not created. Suggested coordinates isn't valid (X: %f Y: %f)",
...
bool Corpse::LoadCorpseFromDB(ObjectGuid::LowType guid, Field* fields)
{
    ...
    if (!IsPositionValid())
    {
        TC_LOG_ERROR(LOG_FILTER_PLAYER, "Corpse (guid: %u, owner: %lu) is not created, given coordinates are not valid (X: %f, Y: %f, Z: %f)",
