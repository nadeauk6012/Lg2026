...
bool EventObject::LoadEventObjectFromDB(ObjectGuid::LowType guid, Map* map)
{
    ...
    if (!data)
    {
        TC_LOG_ERROR(LOG_FILTER_SQL, "EventObject (GUID: %lu) not found in table `eventobject`, can't load. ", guid);
        return false;
    }
...
bool EventObject::Create(ObjectGuid::LowType guidlow, Map* map, uint32 phaseMask, uint32 entry, float x, float y, float z, float ang, float radius, uint32 spell, uint32 worldsafe)
{
    ...
    if (!IsPositionValid())
    {
        TC_LOG_ERROR(LOG_FILTER_UNITS, "EventObject::Create(): given coordinates for eventobject (guidlow %lu, entry %d) are not valid (X: %f, Y: %f, Z: %f, O: %f)", guidlow, entry, x, y, z, ang);
        return false;
    }
