...
void Spell::SelectImplicitBetweenTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType, uint32 effMask)
{
    ...
    if (uint32 containerTypeMask = GetSearcherTypeMask(objectType, condList))
    {
        ...
        TC_LOG_DEBUG(LOG_FILTER_SPELLS_AURAS, "Spell::SelectImplicitBetweenTargets angle %f, dist %f, x %f, y %f, Id %u, targets.size %zu", angle, dist, center->GetPositionX(), center->GetPositionY(), m_spellInfo->Id, targets.size());
...
void Spell::SelectImplicitAreaTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType, uint32 effMask)
{
    ...
    TC_LOG_DEBUG(LOG_FILTER_SPELLS_AURAS, "Spell::SelectImplicitAreaTargets %u, radius %f, GetObjectType %u, targets count %zull, effIndex %i",
        m_spellInfo->Id, radius, targetType.GetObjectType(), targets.size(), effIndex);

    CallScriptObjectAreaTargetSelectHandlers(targets, effIndex, targetType.GetTarget());

    TC_LOG_DEBUG(LOG_FILTER_SPELLS_AURAS, "Spell::SelectImplicitAreaTargets after filter %u, radius %f, GetObjectType %u, targets count %zu, GetCheckType %i, X %f, Y %f",
...
void Spell::cast(bool skipCheck)
{
    if (!m_caster->CheckAndIncreaseCastCounter())
    {
        if (m_triggeredByAuraSpell)
            TC_LOG_DEBUG(LOG_FILTER_GENERAL, "Spell %u triggered by aura spell too deep in cast chain for cast. Cast not allowed for prevent overflow stack crash.", m_spellInfo->Id);
        else
    ...
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
    {
        ...
        switch (auraName)
        {
            ...
            case SPELL_AURA_MOUNTED:
            {
                ...
                if ((m_caster->IsPlayer() && !allowMount && !m_spellInfo->CastingReq.RequiredAreasID) || (m_caster->GetMapId() == 530 && m_caster->ToPlayer()->GetCurrentZoneID() == 0)) //530 - uwow event map
