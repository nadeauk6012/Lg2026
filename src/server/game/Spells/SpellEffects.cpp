...
void Spell::EffectSchoolDMG(SpellEffIndex effIndex)
{
    ...
    TC_LOG_DEBUG(LOG_FILTER_SPELLS_AURAS, "EffectSchoolDMG %i, m_diffMode %i, effIndex %i, spellId %u, damage %f", m_damage, m_diffMode, effIndex, m_spellInfo->Id, damage);
    ...
    if (unitTarget && unitTarget->isAlive())
    {
        ...
        TC_LOG_DEBUG(LOG_FILTER_SPELLS_AURAS, "EffectSchoolDMG end %i, m_diffMode %i, effIndex %i, spellId %u, damage %f GetComboPoints %i", m_damage, m_diffMode, effIndex, m_spellInfo->Id, damage, GetComboPoints());
...
void Spell::EffectHealthLeech(SpellEffIndex effIndex)
{
    ...
    TC_LOG_DEBUG(LOG_FILTER_SPELLS_AURAS, "HealthLeech damage %f bonus %u variance %f", damage, bonus, variance);
    ...
    if (m_caster->isAlive())
    {
        ...
        TC_LOG_DEBUG(LOG_FILTER_SPELLS_AURAS, "HealthLeech damage %f healthGain %lu", damage, healthGain);
...
void Spell::EffectEnergizePct(SpellEffIndex effIndex)
{
    ...
    TC_LOG_DEBUG(LOG_FILTER_SPELLS_AURAS, "Spell::EffectEnergizePct Id %i damage %f power %i gain %i", m_spellInfo->Id, damage, power, gain);
}
    ...
    if (gameObjTarget)
    {
        ...
        switch (gameObjTarget->GetGoType())
        {
            ...
            case GAMEOBJECT_TYPE_CHEST:
                ...
                {
                    TC_LOG_DEBUG(LOG_FILTER_SPELLS_AURAS, "Chest ScriptStart id %u for GO %lu", gameObjTarget->GetGOInfo()->chest.triggeredEvent, gameObjTarget->GetDBTableGUIDLow());
...
void Spell::EffectDisEnchant(SpellEffIndex /*effIndex*/)
{
    ...
    if (GetCaster()->IsPlayer())
        TC_LOG_DEBUG(LOG_FILTER_DONATE, "[Status] Status = 4 item  guid = %u, entry = %u, %s", itemTarget->GetGUID().GetGUIDLow(), itemTarget->GetEntry(), GetCaster()->ToPlayer()->GetInfoForDonate().c_str());
