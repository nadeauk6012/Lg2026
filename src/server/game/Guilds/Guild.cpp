...
void Guild::RankInfo::CreateMissingTabsIfNeeded(uint8 tabs, SQLTransaction& trans, bool logOnCreate /* = false */)
{
    for (uint8 i = 0; i < tabs; ++i)
    {
        ...
        if (logOnCreate)
            TC_LOG_ERROR(LOG_FILTER_GUILD, "Guild %lu has broken Tab %u for rank %u. Created default tab.", m_guildId, i, m_rankId);
...
bool Guild::BankTab::LoadItemFromDB(Field* fields)
{
    ...
    {
        TC_LOG_ERROR(LOG_FILTER_GUILD, "Invalid slot for item (GUID: %lu, id: %u) in guild bank, skipped.", itemGuid, itemEntry);
        return false;
    }
    ...
    if (!proto)
    {
        TC_LOG_ERROR(LOG_FILTER_GUILD, "Unknown item (GUID: %lu, id: %u) in guild bank, skipped.", itemGuid, itemEntry);
        return false;
    }
    ...
    {
        TC_LOG_ERROR(LOG_FILTER_GUILD, "Item (GUID %lu, id: %u) not found in item_instance, deleting from guild bank!", itemGuid, itemEntry);
...
bool Guild::HandleMemberWithdrawMoney(WorldSession* session, uint64 amount, bool repair)
{
    ...
    if (!repair)
    {
        ...
        if (amount >= sWorld->getIntConfig(CONFIG_LOG_GOLD_FROM))
            TC_LOG_DEBUG(LOG_FILTER_GOLD, "GuildWithdrawMoney: %s GUID %u (Account: %u) deposit money (Amount: " UI64FMTD ") to guild bank (Guild ID %lu)", player->GetName(), player->GetGUIDLow(), player->GetSession()->GetAccountId(), amount, m_id);
    }
    // Log guild bank event
    ...
    if (dbTabId < GetPurchasedTabsSize() || isMoneyTab)
    {
        ...
        if (pLog->CanInsert())
        {
            ...
            if (BankEventLogEntry::IsMoneyEvent(eventType))
            {
                if (!isMoneyTab)
                {
                    TC_LOG_ERROR(LOG_FILTER_GUILD, "GuildBankEventLog ERROR: MoneyEvent(LogGuid: %lu, Guild: %lu) does not belong to money tab (%u), ignoring...", guid, m_id, dbTabId);
                    return false;
                }
            }
            else if (isMoneyTab)
            {
                TC_LOG_ERROR(LOG_FILTER_GUILD, "GuildBankEventLog ERROR: non-money event (LogGuid: %lu, Guild: %lu) belongs to money tab, ignoring...", guid, m_id);
                return false;
            }
...
bool Guild::Validate()
{
    ...
    else
    {
        for (uint8 rankId = 0; rankId < _GetRanksSize(); ++rankId)
        {
            ...
            {
                TC_LOG_ERROR(LOG_FILTER_GUILD, "Guild %lu has broken rank id %u, creating default set of ranks...", m_id, rankId);
                broken_ranks = true;
            }
            else
            {
