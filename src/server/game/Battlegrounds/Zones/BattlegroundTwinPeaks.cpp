                ...
                case BG_TP_FLAG_STATE_ON_GROUND:
                    ...
                    if (_flagsDropTimer[team] < 0)
                    {
                        ...
                        if (GameObject* obj = GetBgMap()->GetGameObject(_droppedFlagGUID[team]))
                            ...
                            TC_LOG_ERROR(LOG_FILTER_BATTLEGROUND, "BattlegroundTwinPeaks: An error has occurred in PostUpdateImpl: Unknown dropped flag GUID: %lu", _droppedFlagGUID[team].GetCounter());
        ...
        if (_bothFlagsKept)
        {
            for (uint8 teamTemp = TEAM_ALLIANCE; teamTemp < MAX_TEAMS; ++teamTemp)
            {
                if (Player* pl = ObjectAccessor::FindPlayer(_flagKeepers[team]))
                {
                    ...
                    TC_LOG_ERROR(LOG_FILTER_BATTLEGROUND, "BattlegroundTwinPeaks: An error has occurred in RemovePlayer: player with GUID: %lu haven't been found. (_bothflagsKept is TRUE).", _flagKeepers[team].GetCounter());
            }
