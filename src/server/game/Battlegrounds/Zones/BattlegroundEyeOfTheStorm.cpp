    ...
    for (uint8 i = 0; i < EY_POINTS_MAX; ++i)
    {
        ...
        if (obj)
        {
            ...
            while (j < _playersNearPoint[EY_POINTS_MAX].size())
            {
                ...
                if (!player)
                {
                    TC_LOG_ERROR(LOG_FILTER_BATTLEGROUND, "BattlegroundEyeOfTheStorm:_CheckSomeoneJoinedPoint: Player (GUID: %lu) not found!", _playersNearPoint[EY_POINTS_MAX][j].GetCounter());
                    ++j;
                    continue;
                }
    ...
    for (uint8 i = 0; i < EY_POINTS_MAX; ++i)
    {
        ...
        if (obj)
        {
            ...
            while (j < _playersNearPoint[i].size())
            {
                ...
                if (!player)
                {
                    TC_LOG_ERROR(LOG_FILTER_BATTLEGROUND, "BattlegroundEyeOfTheStorm:_CheckSomeoneLeftPoint Player (GUID: %lu) not found!", _playersNearPoint[i][j].GetCounter());
