...
bool Player::OnSurvey(uint32& entry, float& x, float& y, float& z, float &orientation)
{
	...
	uint8 i = 0;
	std::vector<uint32> research = GetDynamicValues(PLAYER_DYNAMIC_FIELD_RESEARCH_SITES);
	...
	else
	{
		...
		// remove completed site from the player's active research sites
		research.erase(std::remove(research.begin(), research.end(), site_id), research.end());

		// generate a replacement site on the current map to keep the list size stable
		if (uint16 newSite = GenerateResearchSiteInMap(GetMapId()))
			research.push_back(newSite);

		// write changes back to the dynamic field so the client updates immediately
		ClearDynamicValue(PLAYER_DYNAMIC_FIELD_RESEARCH_SITES);
		for (uint32 sid : research)
			AddDynamicValue(PLAYER_DYNAMIC_FIELD_RESEARCH_SITES, sid);
...
void Player::RandomizeSitesInMap(uint32 mapId, uint8 count)
{
	...
	std::vector<uint32> research = GetDynamicValues(PLAYER_DYNAMIC_FIELD_RESEARCH_SITES);
	// Remove all sites in this map
	for (auto itr = research.begin(); itr != research.end(); ++itr)
	{
		uint32 site_id = *itr;
		...
		{
			research.erase(std::remove(research.begin(), research.end(), site_id), research.end());// Delete
		}
	}
	// Generate new sites
	for (uint8 i = 0; i < count; ++i)
	{
		uint16 newsite = GenerateResearchSiteInMap(mapId);
		research.push_back(newsite);
	}
	...
	{
		uint32 site_id = *itr;
		AddDynamicValue(PLAYER_DYNAMIC_FIELD_RESEARCH_SITES, site_id);
	}
	_archaeologyChanged = true;
}
...
void Player::ShowResearchSites()
{
	...
	// Force a rebuild so the client refreshes POIs immediately
	ClearDynamicValue(PLAYER_DYNAMIC_FIELD_RESEARCH_SITES);

	for (uint32 id : research)
	{
...
uint16 Player::GenerateResearchSiteInMap(uint32 mapId)
{
	SiteSet tempSites;
	...
	uint16 site_id = Trinity::Containers::SelectRandomContainerElement(tempSites);
	_archaeologyChanged = true;
	return site_id;
}
...
void Player::GenerateResearchSites()
{
	...
	for (Sites::const_iterator itr = tempSites.begin(); itr != tempSites.end(); ++itr)
	{
		...
		for (uint8 i = 0; i < mapMax;)
		{
			...
			{
				ResearchSiteEntry const* rs = sDB2Manager.GetResearchSiteEntryById(site_id);
				if (!rs || CanResearchWithSkillLevel(rs->ID) == 2)
					site_id = 0;
				if (site_id != 0)
					AddDynamicValue(PLAYER_DYNAMIC_FIELD_RESEARCH_SITES, site_id);
...
void Player::_LoadArchaeology(PreparedQueryResult result)
{
	...
	if (tokens.size() == MAX_RESEARCH_SITES)
	{
		...
		for (size_t i = 0; i < tokens.size(); ++i)
			AddDynamicValue(PLAYER_DYNAMIC_FIELD_RESEARCH_SITES, uint32(atoi(tokens[i])));
	}
	else
		GenerateResearchSites();
