...
void MotionMaster::MoveTargetedHome()
{
    ...
    else if (_owner->GetTypeId() == TYPEID_UNIT && static_cast<Creature*>(_owner)->GetCharmerOrOwnerGUID())
    {
        ...
        if (target)
        {
            TC_LOG_DEBUG(LOG_FILTER_GENERAL, "Following %s (GUID: %lu)", target->IsPlayer() ? "player" : "creature", target->IsPlayer() ? target->GetGUIDLow() : static_cast<Creature*>(target)->GetDBTableGUIDLow());
...
void MotionMaster::MoveChase(Unit* target, float dist, float angle)
{
    ...
    if (_owner->IsPlayer())
    {
        TC_LOG_DEBUG(LOG_FILTER_GENERAL, "Player (GUID: %u) chase to %s (GUID: %lu)", _owner->GetGUIDLow(), target->IsPlayer() ? "player" : "creature", target->IsPlayer() ? target->GetGUIDLow() : target->ToCreature()->GetDBTableGUIDLow());
        Mutate(new ChaseMovementGenerator<Player>(*target, dist, angle), MOTION_SLOT_ACTIVE);
    }
    else
    {
        TC_LOG_DEBUG(LOG_FILTER_GENERAL, "Creature (Entry: %u GUID: %u) chase to %s (GUID: %lu)", _owner->GetEntry(), _owner->GetGUIDLow(), target->IsPlayer() ? "player" : "creature", target->IsPlayer() ? target->GetGUIDLow() : target->ToCreature()->GetDBTableGUIDLow());
...
void MotionMaster::MoveFetch(Unit* target, float dist, float angle, MovementSlot slot)
{
    ...
    if (!_owner->IsPlayer())
    {
        TC_LOG_DEBUG(LOG_FILTER_GENERAL, "Creature (Entry: %u GUID: %u) follow to %s (GUID: %lu)", _owner->GetEntry(), _owner->GetGUIDLow(), target->IsPlayer() ? "player" : "creature", target->IsPlayer() ? target->GetGUIDLow() : target->ToCreature()->GetDBTableGUIDLow());
...
void MotionMaster::MoveFollow(Unit* target, float dist, float angle, MovementSlot slot)
{
    ...
    if (_owner->IsPlayer())
    {
        TC_LOG_DEBUG(LOG_FILTER_GENERAL, "Player (GUID: %u) follow to %s (GUID: %lu)", _owner->GetGUIDLow(), target->IsPlayer() ? "player" : "creature", target->IsPlayer() ? target->GetGUIDLow() : target->ToCreature()->GetDBTableGUIDLow());
...
void MotionMaster::MoveFleeing(Unit* enemy, uint32 time)
{
    ...
    if (_owner->IsPlayer())
    {
        TC_LOG_DEBUG(LOG_FILTER_GENERAL, "Player (GUID: %u) flee from %s (GUID: %lu)", _owner->GetGUIDLow(),
    ...
    {
        TC_LOG_DEBUG(LOG_FILTER_GENERAL, "Creature (Entry: %u GUID: %u) flee from %s (GUID: %lu)%s",
