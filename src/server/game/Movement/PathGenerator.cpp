...
void PathGenerator::BuildPolyPath(G3D::Vector3 const& startPos, G3D::Vector3 const& endPos)
{
    ...
    {
        if (_sourceUnit->IsPlayer() || _sourceUnit->isSummon())
            TC_LOG_DEBUG(LOG_FILTER_PATH_GENERATOR, "++ BuildPolyPath :: (startPoly == %lu || endPoly == %lu)", startPoly, endPoly);
...
void PathGenerator::NormalizePath()
{
    ...
    TC_LOG_DEBUG(LOG_FILTER_PATH_GENERATOR, "PathGenerator::NormalizePath _pathPoints %zu", _pathPoints.size());
    ...
    for (uint32 i = 0; next < _pathPoints.size(); ++i, ++next)
    {
        ...
        while (!result)
        {
            ...
            if (dtVdist(lastPoint, moveTgt) <= 2.0f) // min distance 2m for divide patch
            {
                ...
                dtVcopy(moveCur, nextPoint);
                TC_LOG_DEBUG(LOG_FILTER_PATH_GENERATOR, "NormalizePath 0 push_back %zu vector %f %f %f dist %f 2ddist %f", _newPathPoints.size(), moveTgt[0], moveTgt[1], moveTgt[2], dtVdist(lastPoint, moveCur), dtVdist2D(lastPoint, moveCur));
                break;
            }
            ...
            if (result)
            {
                ...
                TC_LOG_DEBUG(LOG_FILTER_PATH_GENERATOR, "NormalizePath 1 push_back %zu vector %f %f %f dist %f 2ddist %f", _newPathPoints.size(), moveTgt[0], moveTgt[1], moveTgt[2], dtVdist(lastPoint, moveCur), dtVdist2D(lastPoint, moveCur));
    ...
    TC_LOG_DEBUG(LOG_FILTER_PATH_GENERATOR, "PathGenerator::NormalizePath end _pathPoints %zu counter %u", _pathPoints.size(), counter);
}
