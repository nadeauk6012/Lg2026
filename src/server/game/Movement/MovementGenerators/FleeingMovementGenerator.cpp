...
bool FleeingMovementGenerator<T>::_getPoint(T &owner, float &x, float &y, float &z)
{
    ...
    for (uint8 i = 0; i < 18; ++i)
    {
        ...
        float temp_x = x + distance * cos(angle);
        float temp_y = y + distance * sin(angle);
        ...
        if (owner.IsWithinLOS(temp_x, temp_y, z))
        {
            ...
            float new_z = owner.GetHeight(temp_x, temp_y, z, true);
            ...
            if (!(new_z - z) || distance / fabs(new_z - z) > 1.0f)
            {
                ...
                float new_z_left = owner.GetHeight(temp_x + 1.0f*cos(angle+static_cast<float>(M_PI/2)),temp_y + 1.0f*sin(angle+static_cast<float>(M_PI/2)),z,true);
...
void FleeingMovementGenerator<Player>::DoFinalize(Player &owner)
{
    ...
    owner.StopMoving();
    //if (owner.IsPlayer() && unit.IsInWorld())
...
bool TimedFleeingMovementGenerator::DoUpdate(Unit & owner, const uint32& time_diff)
{
    if (!owner.isAlive())
        return false;
