...
bool WaypointMovementGenerator<Creature>::StartMove(Creature &creature)
{
    ...
    if (m_isArrivalDone)
    {
        ...
        // Revers go
        if ((i_currentNode == i_path->size() - 1) && !goBack)
        {
            if (!forceTPToStart)
                goBack = true;
            else
...
{
    if (!i_path || i_path->empty() || i_currentNode >= i_path->size() || m_isArrivalDone)
        return;
    ...
    if (currentPath->event_id && roll_chance_i(currentPath->event_chance))
    {
        TC_LOG_DEBUG(LOG_FILTER_MAPSCRIPTS, "Player movement start script %u at point %u for %lu.", currentPath->event_id, i_currentNode, player.GetGUID().GetCounter());
...
bool WaypointMovementGenerator<Player>::StartMove(Player &player)
{
    ...
    if (m_isArrivalDone)
    {
        if (i_currentNode == i_path->size() - 1) // If that's our last waypoint
        {
...
static float constexpr SKIP_SPLINE_POINT_DISTANCE_SQ = 40.0f * 40.0f;
...
void FlightPathMovementGenerator::PreloadEndGrid()
{
    ...
    if (endMap)
    {
        TC_LOG_INFO(LOG_FILTER_GENERAL, "Preloading rid (%f, %f) for map %u at node index %u/%u", _endGridX, _endGridY, _endMapId, _preloadTargetNode, (uint32)(i_path.size() - 1));
