        ...
        if (!player->HasPlayerExtraFlag(PLAYER_EXTRA_INVISIBLE_STATUS))
        {
            if (player->GetGuildId() != 0)
            {
                ...
                else
                {
                    ...
                    TC_LOG_ERROR(LOG_FILTER_GENERAL, "Player %s (GUID: %u) marked as member of not existing guild (id: %lu), removing guild membership for player.", player->GetName(), player->GetGUIDLow(), player->GetGuildId());
                    player->SetInGuild(0);
                }
            }
        }
...
void WorldSession::HandleCharRaceOrFactionChange(WorldPackets::Character::CharRaceOrFactionChange& packet)
{
    ...
    if (used_loginFlag & AT_LOGIN_CHANGE_FACTION)
    {
        ...
        if (!knownTitlesStr.empty())
        {
            ...
            TC_LOG_DEBUG(LOG_FILTER_UNITS, "Account: %d faction change titles process... tokens size %zu ktcount %u input %s", GetAccountId(), tokens.size(), ktcount, knownTitlesStr.c_str());
    ...
    TC_LOG_DEBUG(LOG_FILTER_UNITS, "Account: %d (IP: %s), Character guid: %lu Change Race/Faction to: %s", GetAccountId(), GetRemoteAddress().c_str(), lowGuid, info->Name.c_str());
