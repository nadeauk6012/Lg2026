    ...
    if (result)
    {
        do
        {
            ...
            if (!Player::ValidateAppearance(charInfo.Race, charInfo.Class, charInfo.Sex, charInfo.HairStyle, charInfo.HairColor, charInfo.Face, charInfo.FacialHair, charInfo.Skin, charInfo.CustomDisplayy))
            {
                ...
                {
                    CharacterDatabaseStatements* stmt = CharacterDatabase.GetPreparedStatements.PreparedStatement*(CHAR_UPD_ADD_AT_LOGIN_FLAG);
...
void WorldSession::HandleCharCreateOpcode(WorldPackets::Character::CreateChar& charCreate)
{
        ...
        .WithChainingPreparedCallback([this, createInfo, SendCharCreate](QueryCallback& queryCallback, PreparedQueryResult result)
    {
        ...
        std::function<void(PreparedQueryResult)> finalizeCharacterCreation = [this, createInfo, SendCharCreate](PreparedQueryResult result)
        {
            ...
            sWorld->AddCharacterInfo(newChar.GetGUID(), GetAccountId(), std::string(newChar.GetName()), newChar.getGender(), newChar.getRace(), newChar.getClass(), newChar.getLevel(), GetAccountId());
            sWorld->UpdateCharacterAccount(newChar.GetGUID(), GetBattlenetAccountId());
...
void WorldSession::HandlePlayerLogin(LoginQueryHolder* holder)
{
    ...
    sWorld->UpdateCharacterAccount(playerGuid, GetBattlenetAccountId());
