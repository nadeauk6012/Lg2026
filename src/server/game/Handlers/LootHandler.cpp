...
void WorldSession::HandleAutostoreLootItemOpcode(WorldPackets::Loot::AutoStoreLootItem& packet)
{
    ...
    for (WorldPackets::Loot::LootRequest const& req : packet.Loot)
    {
        ...
        if (lguid.IsGameObject())
        {
            ...
            loot = &go->loot;
            //TC_LOG_DEBUG(LOG_FILTER_LOOT, "HandleAutostoreLootItemOpcode lguid %lu, pguid %u lguid %lu", lguid, player->personalLoot.GetGUID(), loot->GetGUID());
        }
...
void WorldSession::DoLootRelease(ObjectGuid lguid)
{
    ...
    TC_LOG_DEBUG(LOG_FILTER_NETWORKIO, "WORLD: DoLootRelease lguid %lu", lguid.GetCounter());
...
void WorldSession::HandleMasterLootItem(WorldPackets::Loot::MasterLootItem& packet)
{
    ...
    for (auto const& lootData : packet.Loot)
    {
        ...
        {
            TC_LOG_DEBUG(LOG_FILTER_LOOT, "MasterLootItem: Player %s might be using a hack! (slot %d, size %zu)",
