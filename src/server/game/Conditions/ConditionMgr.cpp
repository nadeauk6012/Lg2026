...
bool ConditionMgr::IsObjectMeetingSmartEventConditions(int64 entryOrGuid, uint32 eventId, uint32 sourceType, Unit* unit, WorldObject* baseObject) const
{
	...
	if (itr != SmartEventConditionStore.end())
	{
		...
		if (i != itr->second.end())
		{
			TC_LOG_DEBUG(LOG_FILTER_CONDITIONSYS, "GetConditionsForSmartEvent: found conditions for Smart Event entry or guid " SI64FMTD " eventId %u", entryOrGuid, eventId);
...
ConditionList ConditionMgr::GetConditionsForSmartEvent(int64 entryOrGuid, uint32 eventId, uint32 sourceType)
{
    ...
    if (itr != SmartEventConditionStore.end())
    {
        ...
        {
            cond = (*i).second;
            TC_LOG_DEBUG(LOG_FILTER_CONDITIONSYS, "GetConditionsForSmartEvent: found conditions for Smart Event entry or guid %ld event_id %u", entryOrGuid, eventId);
        }
    }
    return cond;
}
...
ConditionList ConditionMgr::GetConditionsForPhaseDefinition(uint32 zone, uint32 entry)
{
    ...
    if (itr != PhaseDefinitionsConditionStore.end())
    {
        ...
        {
            cond = (*i).second;
            TC_LOG_DEBUG(LOG_FILTER_CONDITIONSYS, "GetConditionsForPhaseDefinition: found conditions for zone %u entry %u size %zu", zone, entry, cond.size());
        }
    }
...
ConditionList ConditionMgr::GetConditionsForAreaTriggerAction(uint32 areaTriggerId, uint32 actionId)
{
    ...
    if (itr != AreaTriggerConditionStore.end())
    {
        ...
        {
            cond = i->second;
            TC_LOG_DEBUG(LOG_FILTER_CONDITIONSYS, "GetConditionsForAreaTriggerAction: found conditions for areatrigger id %u entry %u", areaTriggerId, actionId);
        }
    }
...
bool ConditionMgr::isConditionTypeValid(Condition* cond)
{
    ...
    switch (cond->ConditionType)
    {
        ...
        case CONDITION_ACTIVE_EVENT:
        {
            ...
            {
                TC_LOG_ERROR(LOG_FILTER_SQL, "ActiveEvent condition has non existing event id (%u), skipped max arr. size %zu", cond->ConditionValue1, events.size());
                return false;
            }
