...
bool LootStoreItem::IsValid(LootStore const& store, uint32 entry) const
{
    ...
    {
        TC_LOG_ERROR(LOG_FILTER_SQL, "Table '%s' entry %d currency %d: group is set, but currencies must not have group - skipped", store.GetName(), entry, itemid);
        return false;
    }
...
void Loot::AddItem(LootStoreItem const & item, std::vector<uint32> const& bonusListIDs)
{
    ...
    if (item.needs_quest)
    {
        ...
        // TC_LOG_DEBUG(LOG_FILTER_LOOT, "Loot::AddItem ItemID %i quest_items %zu", generatedLoot.item.ItemID, quest_items.size());
    }
...
{
    // TC_LOG_DEBUG(LOG_FILTER_LOOT, "Loo::tAutoStoreItems items %zu quest_items %zu", items.size(), quest_items.size());
...
bool Loot::FillLoot(uint32 lootId, LootStore const& store, Player* lootOwner, bool noGroup, bool noEmptyError, WorldObject const* lootFrom)
{
    ...
    else if (isOnlyQuest && (creature && creature->CanShared() || isBoss))
    {
        ...
        // TC_LOG_DEBUG(LOG_FILTER_LOOT, "Loot::FillLoot isOnlyQuest && creature && creature->CanShared size %zu", savethreatlist->size());
...
void Loot::FillNotNormalLootFor(Player* player, bool presentAtLooting)
{
    ...
    // TC_LOG_DEBUG(LOG_FILTER_LOOT, "Loot::FillNotNormalLootFor plguid %u presentAtLooting %lu PlayerQuestItems %zu", plguid, presentAtLooting, PlayerQuestItems.size());
...
{
    // TC_LOG_DEBUG(LOG_FILTER_LOOT, "Loot::FillQuestLoot quest_items %zu isOnlyQuest %u", quest_items.size(), isOnlyQuest);
    ...
    // TC_LOG_DEBUG(LOG_FILTER_LOOT, "Loot::FillQuestLoot ql %zu unlootedCount %u", ql->size(), unlootedCount);
...
void Loot::BuildLootResponse(WorldPackets::Loot::LootResponse& packet, Player* viewer, PermissionTypes permission, ItemQualities threshold /*= ITEM_QUALITY_POOR*/) const
{
    ...
    // TC_LOG_DEBUG(LOG_FILTER_LOOT, "Loot::BuildLootResponse lootPlayerQuestItems size %zu", lootPlayerQuestItems.size());

    QuestItemMap::const_iterator q_itr = lootPlayerQuestItems.find(viewer->GetGUIDLow());
    if (q_itr != lootPlayerQuestItems.end())
    {
        QuestItemList* q_list = q_itr->second;
        // TC_LOG_DEBUG(LOG_FILTER_LOOT, "Loot::BuildLootResponse QuestItemList size %zu", q_list->size());
...
void LootTemplate::LootGroup::Process(Loot& loot, LootTemplate const* tab) const
{
    ...
    // TC_LOG_DEBUG(LOG_FILTER_LOOT, "LootGroup::Process EqualPossibleDrops %u ExplicitPossibleDrops %zu", EqualPossibleDrops.size(), ExplicitPossibleDrops.size());
...
{
    // TC_LOG_DEBUG(LOG_FILTER_LOOT, "LootGroup::LootGroup::ProcessItemLoot EqualChanced %zu", EqualChanced.size());
    ...
    // TC_LOG_DEBUG(LOG_FILTER_LOOT, "LootGroup::LootGroup::ProcessItemLoot ItemPossibleDrops %zu", ItemPossibleDrops.size());
...
void LootTemplate::LootGroup::ProcessWorld(Loot& loot, LootTemplate const* tab, bool ignore) const
{
    ...
    // TC_LOG_DEBUG(LOG_FILTER_LOOT, "LootGroup::ProcessWorld EqualPossibleDrops %zu ignore %u", EqualPossibleDrops.size(), ignore);
    ...
    // TC_LOG_DEBUG(LOG_FILTER_LOOT, "LootGroup::ProcessWorld ItemPossibleDrops %zu ignore %u", ItemPossibleDrops.size(), ignore);
...
bool LootTemplate::LootGroup::ProcessBossLoot(Loot& loot, LootTemplate const* tab) const
{
    ...
    // TC_LOG_DEBUG(LOG_FILTER_LOOT, "LootGroup::ProcessBossLoot EqualChanced %i ExplicitlyChanced %zu", uiMaxAttempts, ExplicitlyChanced.size());
...
bool LootTemplate::LootGroup::ProcessRareOrGoLoot(Loot& loot, LootTemplate const* tab) const
{
    ...
    // TC_LOG_DEBUG(LOG_FILTER_LOOT, "LootGroup::ProcessRareOrGoLoot EqualPossibleDrops %i ExplicitlyChanced %zu", uiMaxAttempts, ExplicitlyChanced.size());
        ...
        else if(item.type == LOOT_ITEM_TYPE_CURRENCY)
        {
            if (item.itemid == 1220 && item.chance < 100) // Order Resources for rare or go have custom group
            {
                ...
                if (group >= PersonalGroups.size())
                    PersonalGroups.resize(group);
                PersonalGroups[group-1].AddEntry(item);
                // TC_LOG_DEBUG(LOG_FILTER_LOOT, "LootTemplate::AddEntry LOOT_ITEM_TYPE_CURRENCY 1220 group %i PersonalGroups %zu", group, PersonalGroups.size());
            }
            Entries.push_back(item);
        }
        else
...
void LootTemplate::Process(Loot& loot, bool rate, uint8 groupId) const
{
    ...
    // TC_LOG_DEBUG(LOG_FILTER_LOOT, "LootTemplate::Process isBoss %i _DifficultyMask %i loot.chance %u bonusLoot %i Entries %zu AutoGroups %zu Groups%zuu", loot.isBoss, loot._DifficultyMask, loot.chance, loot.bonusLoot, Entries.size(), AutoGroups.size(), Groups.size());
...
void LootTemplate::ProcessRareOrGoLoot(Loot& loot) const
{
    ...
    // TC_LOG_DEBUG(LOG_FILTER_LOOT, "LootTemplate::ProcessRareOrGoLoot PersonalGroups %zu", PersonalGroups.size());
...
{
    // TC_LOG_DEBUG(LOG_FILTER_LOOT, "ProcessItemLoot Entries %zu AutoGroups %zu PersonalGroups %zu", Entries.size(), AutoGroups.size(), PersonalGroups.size());
    ...
    // TC_LOG_DEBUG(LOG_FILTER_LOOT, "ProcessItemLoot ItemPossibleDrops %zu APPossibleDrops %zu Groups %zu", ItemPossibleDrops.size(), APPossibleDrops.size(), Groups.size());
...
{
    // TC_LOG_DEBUG(LOG_FILTER_LOOT, "ProcessChallengeChest Entries %zu AutoGroups%zuu PersonalGroups %zu", Entries.size(), AutoGroups.size(), PersonalGroups.size());
    ...
    // TC_LOG_DEBUG(LOG_FILTER_LOOT, "ProcessChallengeChest ItemPossibleDrops %zu", ItemPossibleDrops.size());
    ...
    // TC_LOG_DEBUG(LOG_FILTER_LOOT, "ProcessChallengeChest OtherPossibleDrops %zu", OtherPossibleDrops.size());

    // TC_LOG_DEBUG(LOG_FILTER_LOOT, "ProcessChallengeChest loot.items %zu", loot.items.size());
    uint32 addCA = 0;
    if (_challenge->_complete)
...
void LootMgr::AddLoot(Loot* loot)
{
    ...
    // TC_LOG_DEBUG(LOG_FILTER_LOOT, "LootMgr::AddLoot guid %s size %zu", loot->GetGUID().ToString().c_str(), m_Loots.size());
}
