...
{
    TC_LOG_DEBUG(LOG_FILTER_LOOT, "Loot::AutoStoreItems items %i quest_items %i", items.size(), quest_items.size());
...
QuestItemList* Loot::FillQuestLoot(Player* player)
{
    ...
    for (size_t i = 0; i < quest_items.size(); ++i)
    {
        ...
        TC_LOG_DEBUG(LOG_FILTER_LOOT, "Loot::FillQuestLoot freeforall %u is_blocked %u is_looted %u ItemID %u AllowedForPlayer %u unlootedCount %u", static_cast<bool>(item.freeforall), static_cast<bool>(item.is_blocked), static_cast<bool>(item.is_looted)
