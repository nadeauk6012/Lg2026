...
{
	if ((AccountMgr::IsPlayerAccount(_session->GetSecurity())) || (AccountMgr::IsModeratorAccount(_session->GetSecurity())))
		return true;
...
void BattlepayManager::ProcessDelivery(Purchase* purchase)
{
	...
	case CharacterBoost:
	{
		...
		case GuildRename:
			if (player)
				sCharacterService->GuildRename(player);
			break;
	case Gold1:
		if (player)
	...
	case Level:
		if (player)
			sCharacterService->Level(player);
		break;
	case AppareanceArtifac:
		if (player)
			sCharacterService->AppareanceArtifac(player);
		break;

	case Currency1:
		if (player)
			sCharacterService->Currency1(player);
		break;

	case Currency2:
		if (player)
			sCharacterService->Currency2(player);
		break;
	...
	case ProfSecFish:
		...
		break;
		// Vuelo Draenor y Legion
	case VueloDL:
		if (player)
			sCharacterService->VueloDL(player);
		break;
		// Subir Reputaciones
	case RepClassic:
		if (player)
	...
	case PremadePve:
		if (player)
			sCharacterService->PremadePve(player);
		break;

	case PremadePvp:
		if (player)
			sCharacterService->PremadePvp(player);
		break;

	case Promo:
		if (player)
			sCharacterService->Promo(player);
		break;

	case Battlepay::Gold:
		for (auto const& itr : product.Items)
			if (player)
				player->ModifyCurrency(itr.ItemID, itr.Quantity);
		break;

	case ArtifactPower:
			if (player)
			sCharacterService->ArtifactLevel(player);
		break; 

	case ArtifactPower101:
		if (player)
			sCharacterService->ArtifactPower101(player);
		break;

	case HonorLvl:
		if (player)
			sCharacterService->HonorLvl(player);
		break;
...
std::tuple<bool, WorldPackets::BattlePay::ProductDisplayInfo> BattlepayManager::WriteDisplayInfo(uint32 displayInfoID, LocaleConstant localeIndex, uint32 productId /*= 0*/)
{
	auto GeneratePackDescription = [localeIndex](Product const& product) -> std::string
	{
		...
		for (auto itr : product.Items)
			if (auto itemTemplate = sObjectMgr->GetItemTemplate(itr.ItemID))
				res += getQualityColor(itemTemplate->GetQuality()) + "\n";
		return res;
	};
