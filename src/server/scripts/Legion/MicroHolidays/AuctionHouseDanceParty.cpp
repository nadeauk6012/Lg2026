...
Position Creatures_NightElf[4] =
{
    { -8803.913f, 655.6042f, 99.63034f, 3.681745f },
    { -8809.797f, 667.809f, 99.72423f, 3.681745f },
    { -8805.955f, 658.9271f, 99.72376f, 3.681745f },
    { -8811.285f, 671.7239f, 99.64462f, 3.681745f }
};
...
Position GameObjects_VoidElf[2] =
{
    { -8816.995f, 665.2518f, 99.63415f, 3.561014f },
    { -8812.851f, 655.8577f, 99.74426f, 3.561014f }
};

Position GameObjects_Gnome[2] =
{
    { -8808.56f, 652.0677f, 99.50799f, 3.007043f },
    { -8816.991f, 671.2309f, 99.5046f, 4.018353f }
};

Position GameObjects_NightElf[2] =
{
    { -8817.113f, 670.9358f, 99.51787f, 4.087841f },
    { -8808.811f, 651.9167f, 99.51176f, 2.901476f }
};

Position GameObjects_LighforgedDraenei2[2] =
{
    { -8816.795f, 671.3993f, 99.55019f, 3.818009f },
    { -8808.493f, 651.9739f, 99.58803f, 3.016603f }
};

Position GameObjects_LighforgedDraenei1[3] =
{
    { -8807.811f, 663.507f, 105.4266f, 5.210198f },
    { -8805.941f, 657.2344f, 103.2725f, 5.210198f },
    { -8811.643f, 670.316f, 103.3766f, 5.210198f }
};

struct npc_ashtar : ScriptedAI
{
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (sGameEventMgr->IsActiveEvent(313))
        {
            if (auto auctionist1 = me->FindNearestCreature(44865, 100.0f))
                auctionist1->SetVisible(true);
            if (auto auctionist2 = me->FindNearestCreature(44867, 100.0f))
                auctionist2->SetVisible(true);
            if (auto auctionist3 = me->FindNearestCreature(44866, 100.0f))
                auctionist3->SetVisible(true);
            if (auto auctionist4 = me->FindNearestCreature(44868, 100.0f))
                auctionist4->SetVisible(true);
            if (auto guard1 = me->FindNearestCreature(44871, 100.0f))
            ...
            sGameEventMgr->StopEvent(313, true);
            sGameEventMgr->StopEvent(300, true);
        }
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_1:
            {
                ...
                std::map<uint8, std::list<Player*>> playerRace[MAX_TEAMS];
                ...
                Trinity::Containers::RandomResizeList(listPl, 14);
    ...
    void DoAction(int32 const action) override
    {
        ...
        case ACTION_1:
            ...
            for (uint8 i = 0; i < 2; ++i)
            {
                me->SummonGameObject(247599, GameObjects_PandaHorde1[i], 0, 0, 0, 0, 0, 15);
                me->SummonGameObject(247600, GameObjects_PandaHorde2[i], 0, 0, 0, 0, 0, 15);
        ...
        case ACTION_2:
            ...
            for (uint8 i = 0; i < 2; ++i)
            {
                me->SummonGameObject(269856, GameObjects_Troll[i], 0, 0, 0, 0, 0, 15);
        ...
        case ACTION_3:
            Talk(SAY_TAUREN);
            for (uint8 i = 0; i < 2; ++i)
                me->SummonGameObject(269857, GameObjects_Tauren[i], 0, 0, 0, 0, 0, 15);
            break;
        case ACTION_4:
            Talk(SAY_ORC);
            for (uint8 i = 0; i < 4; ++i)
                me->SummonGameObject(269861, GameObjects_Orc[i], 0, 0, 0, 0, 0, 15);
            break;
        case ACTION_5:
            Talk(SAY_GOBLIN);
            for (uint8 i = 0; i < 2; ++i)
            {
                me->SummonGameObject(269858, GameObjects_Goblin1[i], 0, 0, 0, 0, 0, 15);
                me->SummonGameObject(269869, GameObjects_Goblin2[i], 0, 0, 0, 0, 0, 15);
            }
        ...
        case ACTION_6:
            ...
            for (uint8 i = 0; i < 2; ++i)
            {
                me->SummonGameObject(269855, GameObjects_Undead[i], 0, 0, 0, 0, 0, 15);
        ...
        case ACTION_7:
            ...
            for (uint8 i = 0; i < 2; ++i)
            {
                me->SummonGameObject(269859, GameObjects_BloodElf[i], 0, 0, 0, 0, 0, 15);
...
struct npc_marla : ScriptedAI
{
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_1:
            {
                ...
                std::list<Player*> listPl = selectList.second;
                Trinity::Containers::RandomResizeList(listPl, 26);
                uint8 selectRace = selectList.first;
    ...
    void DoAction(int32 const action) override
    {
        ...
        case ACTION_2:
            ...
            for (uint8 i = 0; i < 2; ++i)
                me->SummonGameObject(269872, GameObjects_Human[i], 0, 0, 0, 0, 0, 15);
            break;
        case ACTION_3:
            Talk(SAY_DRAENEI);
            ...
            for (uint8 i = 0; i < 2; ++i)
            {
                me->SummonGameObject(269878, GameObjects_Draenei1[i], 0, 0, 0, 0, 0, 15);
                me->SummonGameObject(269878, GameObjects_Draenei2[i], 0, 0, 0, 0, 0, 15);
            }
            break;
        case ACTION_4:
            Talk(SAY_GNOME);
            for (uint8 i = 0; i < 2; ++i)
                me->SummonGameObject(269877, GameObjects_Gnome[i], 0, 0, 0, 0, 0, 15);

            me->SummonCreature(122070, -8811.37f, 669.3073f, 99.70203f, 3.573435f, TEMPSUMMON_TIMED_DESPAWN, 15000);
            me->SummonCreature(122070, -8805.462f, 657.0504f, 99.67287f, 3.460299f, TEMPSUMMON_TIMED_DESPAWN, 15000);
            break;
        case ACTION_5:
            Talk(SAY_DWARF);
            break;
        case ACTION_6:
            Talk(SAY_NIGHTELF);
            for (uint8 i = 0; i < 2; ++i)
                me->SummonGameObject(269876, GameObjects_NightElf[i], 0, 0, 0, 0, 0, 15);
            for (uint8 i = 0; i < 4; ++i)
                me->SummonCreature(122069, Creatures_NightElf[i], TEMPSUMMON_TIMED_DESPAWN, 15000);
            break;
        case ACTION_7:
            Talk(SAY_WORGEN);
            for (uint8 i = 0; i < 2; ++i)
                me->SummonGameObject(269879, GameObjects_Worgen[i], 0, 0, 0, 0, 0, 15);
            break;
        case ACTION_8:
            Talk(SAY_LIGHTFORGED_DRAENEI);
            for (uint8 i = 0; i < 3; ++i)
                me->SummonGameObject(281270, GameObjects_LighforgedDraenei1[i], 0, 0, 0, 0, 0, 15);
            for (uint8 i = 0; i < 2; ++i)
                me->SummonGameObject(281270, GameObjects_LighforgedDraenei2[i], 0, 0, 0, 0, 0, 15);
            break;
        case ACTION_9:
            Talk(SAY_VOID_ELF);
            for (uint8 i = 0; i < 2; ++i)
                me->SummonGameObject(281271, GameObjects_VoidElf[i], 0, 0, 0, 0, 0, 15);
            break;
        }
    }
};
 
class eventobject_dance_studio_orgrimmar : public EventObjectScript
{
public:
    eventobject_dance_studio_orgrimmar() : EventObjectScript("eventobject_dance_studio_orgrimmar") {}
...
class eventobject_dance_studio_stormwind : public EventObjectScript
{
public:
    eventobject_dance_studio_stormwind() : EventObjectScript("eventobject_dance_studio_stormwind") {}
...
class spell_teleport_off_stage_horde : public SpellScript
{
    ...
    {
        OnEffectHitTarget += SpellEffectFn(spell_teleport_off_stage_horde::HandleScriptEffect, EFFECT_0, SPELL_EFFECT_TELEPORT_L);
    }
};
...
class spell_teleport_off_stage_alliance : public SpellScript
{
    ...
    void Register() override
    {
        OnEffectHitTarget += SpellEffectFn(spell_teleport_off_stage_alliance::HandleScriptEffect, EFFECT_0, SPELL_EFFECT_TELEPORT_L);
    }
};

void AddSC_AuctionHouseDanceParty()
{
    RegisterCreatureAI(npc_ashtar);
    RegisterCreatureAI(npc_marla);
    RegisterSpellScript(spell_teleport_off_stage_horde);
    RegisterSpellScript(spell_teleport_off_stage_alliance);
    new eventobject_dance_studio_orgrimmar();
    new eventobject_dance_studio_stormwind();
}