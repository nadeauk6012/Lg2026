...
#include "ScriptedCreature.h"
  ...
  //     }
  // };
	 // 254798 Reparar Usado para el teleport y el control de las areas por spell_area 
...
public:
	...
	class spell_principles_of_war_AuraScript : public AuraScript
	{
		...
		void CalculateAmount(AuraEffect const* /*aurEff*/, float& amount, bool& canBeRecalculated)
		{
			...
			float itemLevel = player->GetAverageItemLevelEquipped();
			if (itemLevel > 1000.f)
				itemLevel = 1000.f;
			...
			// Sometimes players continus flying
...
class spell_gen_spec_stat_template : public AuraScript
{
	...
	void CalculateAmount(AuraEffect const* aurEff, float& amount, bool& /*canBeRecalculated*/)
	{
		if (Unit* owner = GetUnitOwner())
		{
			if (Player* plr = owner->ToPlayer())
			{
				...
				auto PvpMain1 = sWorld->getIntConfig(CONFIG_PVP_TEMPLATE_MAIN_STAD);
				auto PvpMain2 = sWorld->getIntConfig(CONFIG_PVP_TEMPLATE_STAMINA_STAD);
				auto PvpMain3 = sWorld->getIntConfig(CONFIG_PVP_TEMPLATE_SECUNDARY_STAD);
				float statMod = 0.f;
				...
				{
				case EFFECT_0: // ATTACK BONUS POWER
				{
					amount = 0;
					break;
				}
				case EFFECT_1: // MOD HEALING DONE PERCENT
				{
					amount = sWorld->GetCustomPvpPenalizations(specId, CUSTOM_MOD_HEAL_DONE);
					break;
				}
				case EFFECT_2: // MOD DAMAGE DONE PERCENT
				{
					amount = statMod;
					amount += sWorld->GetCustomPvpPenalizations(specId, CUSTOM_MOD_DAMAGE_DONE);
					break;
				}
				case EFFECT_3: // MANA REGEN
				{
					amount = statMod;
					break;
				}
				case EFFECT_4: // STRENGTH
				case EFFECT_5: // AGILITY
				case EFFECT_6: // INTELLECT
				{
					auto PvpMainStad = sWorld->getIntConfig(CONFIG_PVP_TEMPLATE_MAIN_STAD);
					amount = (plr->GetCreateStat(Stats(aurEff->GetMiscValue())) + PvpMainStad) * statMod;
					break;
				}
				case EFFECT_7: // STAMINA
				{
					auto PvpStaminaStad = sWorld->getIntConfig(CONFIG_PVP_TEMPLATE_STAMINA_STAD);
					amount = (plr->GetCreateStat(Stats(aurEff->GetMiscValue())) + PvpStaminaStad) * statMod;
					break;
				}
				case EFFECT_8:  // ARMOR
				{
					float PvpArmor = plr->GetPvpArmorTemplate(specId);
					amount = PvpArmor * statMod - PvpArmor;
					break;
				}
				case EFFECT_9:  // MASTERY
				{
					auto PvpSecondaryStad = sWorld->getIntConfig(CONFIG_PVP_TEMPLATE_SECUNDARY_STAD);
					float value = sWorld->GetCustomPvpPenalizations(specId, CUSTOM_MOD_MASTERY);
					value /= 100;
					amount = PvpSecondaryStad * (statMod + value) - PvpSecondaryStad;
					break;
				}
				case EFFECT_10: // HASTE
				case EFFECT_11: // VERSATILITY DMG DONE
				case EFFECT_12: // VERSATILITY DMG TAKEN
				case EFFECT_13: // CRIT
				{
					auto PvpSecondaryStad = sWorld->getIntConfig(CONFIG_PVP_TEMPLATE_SECUNDARY_STAD);
					amount = PvpSecondaryStad * statMod - PvpSecondaryStad;
					break;
				}
				}
			}
	...
	void Register() override
	{
		...
		DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_gen_spec_stat_template::CalculateAmount, EFFECT_1, SPELL_AURA_MOD_HEALING_DONE_PERCENT);
...
// quest 44821 Dalaran Legion Chain
enum Quest44821Misc
{
	KHADGAR_SAY_0 = 0,
	KHADGAR_SAY_1 = 1,
	...
	MODERA_SAY_4 = 4,
	MODERA_SAY_5 = 5,
	QUEST_ID_IN_DIRE_NEED = 44821,
	SPELL_KILL_CREDIT_QUEST_44821 = 229648,
	NPC_ARCHIMAGE_MODERA = 115465,
	NPC_ARCHIMAGE_KHADGAR = 72646,
};

struct npc_archimage_115465 : public ScriptedAI
{
	npc_archimage_115465(Creature* creature) : ScriptedAI(creature) {}

	void sQuestAccept(Player* player, Quest const* quest) override
	{
		if (quest->Id == QUEST_ID_IN_DIRE_NEED)
		{
			Talk(MODERA_SAY_4);
			...
			AddDelayedEvent(6000, [this]() -> void
			{
				...
				{
					if (auto astralaire = me->FindNearestCreature(NPC_ARCHIMAGE_KHADGAR, 50.0f, true))
						astralaire->GetAI()->DoAction(KHADGAR_SAY_0);
				});
			});
		}
	}
...
struct npc_archimage_72646 : public ScriptedAI
{
	npc_archimage_72646(Creature* creature) : ScriptedAI(creature) {}

	void DoAction(int32 const action) override
	{
		if (action == KHADGAR_SAY_0)
		{
			Talk(KHADGAR_SAY_0);
			AddDelayedEvent(10000, [this]() -> void
			{
				Talk(KHADGAR_SAY_1);
				AddDelayedEvent(8000, [this]() -> void
				{
					Talk(KHADGAR_SAY_2);
					AddDelayedEvent(8000, [this]() -> void
					{
						...
						if (!Players.empty())
							...
							{
								player->CastSpell(player, SPELL_KILL_CREDIT_QUEST_44821, true);
							}
					});
				});
			});
...
// ID - 227021 Visions of the Dark Titan
class spell_dk_visions_of_the_dark_titan : public SpellScriptLoader
{
public:
	spell_dk_visions_of_the_dark_titan() : SpellScriptLoader("spell_dk_visions_of_the_dark_titan") { }

	class spell_dk_visions_of_the_dark_titan_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_dk_visions_of_the_dark_titan_SpellScript);
		...
		void Register() override
		{
			OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_dk_visions_of_the_dark_titan_SpellScript::FilterTargets, EFFECT_0, TARGET_UNIT_SRC_AREA_ENEMY);
		}
	};

	SpellScript* GetSpellScript() const override
	{
		return new spell_dk_visions_of_the_dark_titan_SpellScript();
	}
};
...
void AddSC_generic_spell_scripts()
{
	...
	RegisterCreatureAI(npc_archimage_115465);
	RegisterCreatureAI(npc_archimage_72646);

	new spell_dk_visions_of_the_dark_titan();
}
