...
public:
    ...
    struct boss_warrior_toc5AI : public npc_mounted_championAI
    {
        ...
        void DamageTaken(Unit* /*attacker*/, uint32 & damage, DamageEffectType dmgType) override
        {
            if (isInMountedGauntlet())
            {
                npc_mounted_championAI::DamageTaken(NULL, damage, dmgType);
                return;
            }
    ...
    public:
        ...
        class spell_toc5_ride_mount_SpellScript : public SpellScript
        {
            ...
            SpellCastResult CheckRequirement()
            {
                ...
                {
                    if (auto item = player->GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_MAINHAND))
                    {
                        if (item->GetEntry() == 46106 || item->GetEntry() == 46069 || item->GetEntry() == 46070)
                        {
